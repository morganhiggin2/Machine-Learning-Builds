...
#add residual term in network, and in blocks of deep cnns
#test and finish
class ConvoClassifier(torch.nn.Module):
    def __init__(self, num_channels, kern_d1, kern_d2):
        super().__init__()

        self.convo1 = torch.nn.Conv2d(in_channels=num_channels, out_channels=num_channels, kernel_size=(kern_d1, kern_d2), padding='same', padding_mode='zeros')
        self.maxPool2d = torch.nn.MaxPool2d(kernel_size=(3, 3), stride=1, padding=1, ceil_mode=False)
        self.convo2 = torch.nn.Conv2d(in_channels=num_channels, out_channels=1, kernel_size=(3, 3), padding='same', padding_mode='zeros')
        self.batchNormalization = torch.nn.BatchNorm2d(num_channels)
        
        self.net = torch.nn.Sequential(self.convo1, self.batchNormalization, torch.nn.ReLU(), self.maxPool2d, self.convo2)
...
pandas_predicted_values.to_csv("data/image_convo_2d_3_output.csv")
...
pred_temp = torch.clamp(torch.abs(predicted_targets[0] * 255), min=0.0, max=255.0).type(torch.uint8)
...
predicted_image.save("data/image_convo_2d_3_output.png")
